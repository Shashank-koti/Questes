    ...
    >
      <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-baseline cursor-pointer group">
          <span className="font-serif font-black text-3xl md:text-4xl tracking-tight text-brand-primary transition-transform duration-300 group-hover:scale-105">Q</span>
        ...
        <nav className="hidden lg:flex items-center gap-8">
          ...
          <Link to="/contact" className="bg-brand-primary hover:bg-brand-secondary text-white px-6 py-2.5 rounded-full text-sm font-bold shadow-[0_4px_14px_rgba(223,30,97,0.35)] hover:shadow-[0_6px_20px_rgba(223,30,97,0.5)] transition-all duration-300 transform hover:-translate-y-0.5">
            Partner With Us
          </Link>
        </nav>
      ...
      {/* Mobile Navigation */}
      <div
        className={`lg:hidden absolute top-full left-0 w-full bg-white shadow-xl border-b border-brand-border transition-all duration-300 overflow-hidden ${isOpen ? 'max-h-[500px] py-4' : 'max-h-0 py-0'
          }`}
      >
        <div className="flex flex-col px-4 gap-2">
          {navLinks.map((link) => (
            <div key={link.name} className="flex flex-col">
              {link.hasDropdown ? (
                <div
                  className="py-3 font-semibold text-brand-dark border-b border-brand-light flex justify-between items-center cursor-pointer"
                >
                  {link.name}
              ...
              {link.hasDropdown && (
                <div className="flex flex-col pl-4 py-2 gap-2 bg-brand-light/50 rounded-b-lg">
                  {link.items.map((item, idx) => (
                    <Link
                      key={idx}
                      to={item.to}
                      className="flex items-center py-2 text-sm font-medium text-brand-dark"
                      onClick={() => setIsOpen(false)}
                    >
                      {item.icon}
                      {item.name}
          ...
          <Link to="/contact" onClick={() => setIsOpen(false)} className="mt-4 bg-brand-primary text-white py-3 rounded-xl font-bold w-full text-center shadow-md inline-block">
            Partner With Us
          </Link>
        </div>
      </div>
